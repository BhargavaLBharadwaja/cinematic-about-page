...
export class SceneManager {
  ...
  readonly astronaut: AstronautController; readonly particleSystem: ParticleSystem; readonly atmosphere: AtmosphereSystem; readonly portal: PortalSystem; readonly glass: GlassSystem; readonly expertise = new ExpertiseController();
  private readonly connectors = new THREE.Group(); private readonly connectorGeometry = new THREE.TorusGeometry(.5, .13, 12, 32);
  constructor(private quality: { particleScale: number; dpr?: number }) {
    this.root.add(this.connectors); this.astronaut = new AstronautController(); this.particleSystem = new ParticleSystem(quality); this.atmosphere = new AtmosphereSystem(quality); this.portal = new PortalSystem(quality); this.glass = new GlassSystem(quality);
  ...
  setExpertiseIndex(index: number) { this.expertise.setIndex(index); }
  update(state: ScrollState, delta: number, pointer: THREE.Vector2, reducedMotion: boolean) {
    const section = state.sections[state.activeSection]; const scene = section?.scene ?? 'connector'; const expertise = this.expertise.update(delta, reducedMotion);
    ...
    this.particleSystem.update(state, delta, pointer, reducedMotion, expertise);
    this.atmosphere.update(state, delta, pointer, reducedMotion, expertise);
    this.portal.update(section?.progress ?? 0, state, delta, reducedMotion); this.glass.update(section?.progress ?? 0, state, delta, reducedMotion); this.astronaut.update(state, delta, pointer, reducedMotion);
  }
