...
export class SceneManager {
  ...
  constructor(private quality: { particleScale: number; dpr?: number }) {
    ...
    this.root.add(this.particleSystem.root, this.atmosphere.root, this.astronaut.root);
    const connectorGeometry = new THREE.TorusGeometry(.5, .13, 12, 32);
    const colors = [0x6b7cff, 0x17191f, 0xf4f3ef, 0x8d55ff];
    for (let i = 0; i < 40; i += 1) { const material = new THREE.MeshStandardMaterial({ color: colors[i % colors.length], roughness: .28, metalness: .18 }); const mesh = new THREE.Mesh(connectorGeometry, material); mesh.position.set((Math.random() - .5) * 5, (Math.random() - .5) * 4, (Math.random() - .5) * 3); mesh.rotation.set(Math.random() * 3, Math.random() * 3, Math.random() * 3); mesh.scale.setScalar(.45 + Math.random() * .7); this.connectors.add(mesh); }
  }
  ...
  update(state: ScrollState, delta: number, pointer: THREE.Vector2, reducedMotion: boolean) {
    ...
    this.atmosphere.update(state, delta, reducedMotion);
    this.astronaut.update(state, delta, pointer, reducedMotion);
    this.root.position.y += (-state.velocity * .00015 - this.root.position.y) * .04;
  }

  resize(dpr: number) { this.particleSystem.resize(dpr); }
  dispose() { this.astronaut.dispose(); this.particleSystem.dispose(); this.atmosphere.dispose(); this.root.traverse((object) => { const mesh = object as THREE.Mesh; if (mesh.geometry) mesh.geometry.dispose(); if (Array.isArray(mesh.material)) mesh.material.forEach((material) => material.dispose()); else if (mesh.material) mesh.material.dispose(); }); }
}
